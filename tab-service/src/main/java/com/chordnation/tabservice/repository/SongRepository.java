...
public interface SongRepository extends JpaRepository<Song, Long> {
    @Query("SELECT s FROM Song s JOIN s.tabs t WHERE (s.artist IN (:artists)) OR (s.genre IN (:genres)) AND (t.level = :level AND t.guitarType = :guitarType)")
    List<Song> findAllSuggestedForUser(List<String> artists, List<String> genres, Level level, GuitarType guitarType);
